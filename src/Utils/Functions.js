...
import vlp_ABI from "../Utils/ABI/vlp_ABI.json"
import State_abi from '../Utils/ABI/state_token.json'
import { PSD_ADDRESS, state_token, allInOnePopup, vlp_ADDRESS } from './ADDRESSES/Addresses';
...
export default function Functions({ children }) {
    ...
    const getVLPContract = async () => {
        try {
            const provider = await getProvider();
            const signer = provider.getSigner();
            const vlp_Contract = new ethers.Contract(vlp_ADDRESS, vlp_ABI, signer);
    ...
    const approve = async (amount) => {
        try {
            // Display a popup message indicating the start of the deposit process
            allInOnePopup(null, 'Approving vlpPLS', null, `OK`, null);
            ...
            let vlpContract = await getVLPContract();


            // First, approve the contract to spend the specified amount on behalf of the user
            const approveTx = await vlpContract?.approving(accountAddress, vlpContract.address, parsedAmount);
    ...
    const handleDeposit = async (amount) => {
        ...
        try {
            ...
            // First, approve the contract to spend the specified amount on behalf of the user
            const approveTx = await contract?.approving(parsedAmount);
            await approveTx.wait();


            allInOnePopup(null, 'Create a New Vault', null, `OK`, null);
            // Then, proceed with the deposit
    ...
    const CreateTargets = async (amount) => {
        console.log('amount:', amount);

        try {
            // Displaying initial popup
            allInOnePopup(null, 'Create a New Vault', null, `OK`, null);

            // Parse the amount to the correct format (assuming amount is in Ether units)
            const parsedAmount = ethers.utils.parseEther(amount.toString());

            // Get the contract instance
            let contract = await getPsdContract();

            // Call the createTargets function with the specified amount
            let createTargetsTx = await contract.createTargets(parsedAmount);
            await createTargetsTx.wait();

            // Display success popup
            allInOnePopup(null, 'Vaults value locked', null, `OK`, null);
            console.log('createTargetsTx:', createTargetsTx);

            // Update the socket or any state if required
            setSocket(prevBool => !prevBool);
            return true;
        } catch (error) {
            // Display error popup
    ...
    const BurnAmount = async (amount) => {
        console.log('amount:', amount);

        try {
            // Displaying initial popup
            allInOnePopup(null, 'Burn Tokens ', null, `OK`, null);
            ...
            // Call the createTargets function with the specified amount
            let createTargetsTx = await contract.burn(parsedAmount);
            await createTargetsTx.wait();

            // Display success popup
            allInOnePopup(null, 'Vaults Values are available for claim.', null, `OK`, null);
            console.log('createTargetsTx:', createTargetsTx);
    ...
    const balanceOFMint = async (address) => {
        try {
            const contract = await getPsdContract();
            const balanceOF = await contract?.getUserTokenBalance(address);
            const value = ethers.utils.formatEther(balanceOF.toString());
            const formattedTotalSupply = Number(value);
            console.log("total value", formattedTotalSupply)
    ...
    const DistributedAmount = async (address) => {
        try {
            let contract = await getPsdContract();
            let dia = contract.DistributedAmount(address);
            const value = ethers.utils.formatEther(dia.toString());
            const formatedValue = Number(value);
            console.log("DISTRIBUTED AMOUNT", formatedValue)
    ...
    const burnedAmounts = async (accountAddress) => {
        try {
            if (accountAddress) {
                let contract = await getPsdContract()
                let userBucketBalance = await contract.burnedAmounts(accountAddress)
                let BucketInStr = await userBucketBalance.toString()
                console.log("burned amounts:", BucketInStr)
                return (BucketInStr)
            }
        } catch (error) {
            console.error('getToBeClaimed error:', error);
    ...
    const getUserDistributedTokens = async (address) => {
        try {
            ...
            let distributedTokens = await contract.calculateTotalReachedTargetAmount(address);
