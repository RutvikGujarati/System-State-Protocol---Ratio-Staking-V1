...
contract vlpPLSToken is ERC20, Ownable(msg.sender) {
    ...
    mapping(address => uint256) public bpm;

    function BalancePerMint(uint256 amount) public returns (uint256) {
        // Get the balance before minting
        uint256 balanceBeforeMint = balanceOf(msg.sender);

        // Mint the vlpPLS tokens to the sender
        _mint(msg.sender, amount);

        // Get the balance after minting
        uint256 balanceAfterMint = balanceOf(msg.sender);

        uint256 currentMintedTokens = balanceAfterMint.sub(balanceBeforeMint);
        currentMintedTokens += bpm[msg.sender];
...
contract System_State_Ratio_Vaults_V1 is Ownable(msg.sender) {
    ...
    constructor() {
        ...
        priceFeed = PLSTokenPriceFeed(
            0xfd71C1d7f1d3582A1de5102eA2a25988FB153E69
        );
        token = vlpPLSToken(0x561f005dE1F8623bcf25415b8f161388D7944eAC);
        vlpPLS = IERC20(0x561f005dE1F8623bcf25415b8f161388D7944eAC);
    ...
    function calculationFunction(
        ...
        uint256 ratioPriceTarget = (value).mul(400).div(1000); // Increment Price Target (iPT) Escrow Vaults - 40%
        ...
        uint256 ProtocolFees = (value).mul(50).div(1000); // Protocol Fee - 5%
    ...
    function deposit() public payable {
        ...
        priceFeed.updatePrice(priceFeed.getPrice().add(incrementSize));
    ...
    function vlpPLSToWei(uint256 vlpPLSTokens) public view returns (uint256) {
        // Get the decimals of the token (assuming 18 decimals for this example)
        uint8 decimals = token.decimals();

        // Calculate the value in wei
        return vlpPLSTokens.mul(10 ** decimals);
    }
    ...
    function weiToVlpPLSTokens(
        uint256 weiAmount
    ) public view returns (uint256) {
        return weiAmount / mintPrice;
    }

    function burn(address user) public {
        uint256 burnAMount = calculateTotalReachedTargetAmount(user);

        uint256 plsRewardAmount = burnAMount.mul(mintPrice).div(1 ether);

        PSTClaimed[user] = PSTClaimed[user].add(plsRewardAmount);
        token.burn(msg.sender, burnAMount);

        uint256 decrementSize = burnAMount.div(10000); // Adjust the divisor as needed to control the increment size

        // Increment the global mint price
        mintPrice = mintPrice.add(decrementSize);
        priceFeed.updatePrice(priceFeed.getPrice().add(decrementSize));
    }
    ...
    function claimAllReward() public {
        ...
        Target[] storage userTarget = targetMapping[user];

        for (uint256 i = 0; i < userTarget.length; i++) {
            if (
                userTarget[i].isClosed &&
                price() >= userTarget[i].ratioPriceTarget &&
    ...
    function calculateTotalReachedTargetAmount(
        ...
        Target[] storage userTargetss = targetMapping[user];

        uint256 totalReachedTargetAmount = 0;

        for (uint256 i = 0; i < userTargetss.length; i++) {
            if (
                userTargetss[i].isClosed &&
                price() >= userTargetss[i].ratioPriceTarget &&
                !claimedTargets[user][i]
            ) {
                totalReachedTargetAmount = totalReachedTargetAmount.add(
                    userTargetss[i].TargetAmount
                );
            }
        }
